...
export default function sitemaxPlugin(options = {}) {
  ...
  return {
    ...
    transformIndexHtml(html) {
      ...
      if (meta.ogImage)
        headTags += `<meta property="og:image" content="${meta.ogImage}"/>`;
      if (meta.title)
        headTags += `<meta property="og:title" content="${meta.title}"/>`;
      if (meta.description)
        headTags += `<meta property="og:description" content="${meta.description}"/>`;
    ...
    async closeBundle() {
      ...
      const urls = files.map((f) => {
        ...
        // avoid double-slash if siteUrl ends with /
